...
void serve_server_error(int fd) {
    http_start_response(fd, 500);
    http_send_header(fd, "Content-Type", "text/html");
    http_end_headers(fd);
}

void serve_redirect(int fd, char *path) {
    http_start_response(fd, 302);
    http_send_header(fd, "Content-Type", "text/html");
    http_send_header(fd, "Location", path);
    http_end_headers(fd);
}
...
void serve_file(int fd, char *path) {
    ...
    char file_size[20];
    snprintf(file_size, 20, "%ld", file_stat.st_size);

    int file_fd = open(path, O_RDONLY);
    if (file_fd < 0) {
        serve_server_error(fd);
        return;
    }
...
void serve_directory(int fd, char *path) {
    ...
    snprintf(buffer, 20, "%d", content_length);
    ...
    while ((pdirent = readdir(pdir)) != NULL) {
        ...
        http_format_href(buffer, path, pdirent->d_name);
        dprintf(fd, "%s\r\n", buffer);
    }
...
void handle_files_request(int fd) {
    ...
    path[1] = '/';
    int offset = 0;
    while (request->path[offset] == '/') {
        offset += 1;
    }
    memcpy(path + 2, request->path + offset, strlen(request->path + offset) + 1);
    printf("path: %s\n", path);
    /*
